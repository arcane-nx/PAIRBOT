...
// In-memory storage for chat history and user info with TTL
const chatMemory = {
    messages: new Map(), // Stores last 20 messages per user
    userInfo: new Map(),  // Stores user information
    lastAccess: new Map() // Stores last access time for cleanup
};
...
function cleanupMemory() {
    const now = Date.now();
    for (const [userId, lastTime] of chatMemory.lastAccess.entries()) {
        if (now - lastTime > MEMORY_TTL) {
            chatMemory.messages.delete(userId);
            chatMemory.userInfo.delete(userId);
            chatMemory.lastAccess.delete(userId);
        }
    }
}
    ...
    if (isOwner) {
        if (match === 'on') {
            ...
            if (data.chatbot[chatId]) {
                return sock.sendMessage(chatId, { 
                    text: '* ❤️‍🔥🎊𝐄𝐌𝐌𝐘𝐇𝐄𝐍𝐙-𝐕3.1🎊❤️‍🔥 Chatbot is already enabled for this group*',
                    quoted: message
                });
            }
            data.chatbot[chatId] = true;
            saveUserGroupData(data);
            console.log(`❤️‍🔥🎊𝐄𝐌𝐌𝐘𝐇𝐄𝐍𝐙-𝐕3.1🎊❤️‍🔥 Chatbot enabled for group ${chatId}`);
            return sock.sendMessage(chatId, { 
                text: '*❤️‍🔥🎊𝐄𝐌𝐌𝐘𝐇𝐄𝐍𝐙-𝐕3.1🎊❤️‍🔥 Chatbot has been enabled for this group*',
                quoted: message
            });
        }
        ...
        if (match === 'off') {
            ...
            if (!data.chatbot[chatId]) {
                return sock.sendMessage(chatId, { 
                    text: '*❤️‍🔥🎊𝐄𝐌𝐌𝐘𝐇𝐄𝐍𝐙-𝐕3.1🎊❤️‍🔥 Chatbot is already disabled for this group*',
                    quoted: message
                });
            }
...
async function handleChatbotResponse(sock, chatId, message, userMessage, senderId) {
    ...
    try {
        ...
        chatMemory.lastAccess.set(senderId, Date.now());
