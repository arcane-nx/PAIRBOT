...
async function stickerCommand(sock, chatId, message) {
  ...
  const mediaMessage =
    targetMessage.message?.imageMessage || targetMessage.message?.videoMessage || targetMessage.message?.documentMessage
  ...
  try {
    ...
    try {
      ...
      console.log("Primary download method failed, trying alternative...")
      // Fallback method
      try {
        ...
        throw new Error("All download methods failed", { cause: fallbackError });
      }
    }
    ...
    const isAnimated =
      mediaMessage.mimetype?.includes("gif") || mediaMessage.mimetype?.includes("video") || mediaMessage.seconds > 0
...
const originalCommand = stickerCommand
