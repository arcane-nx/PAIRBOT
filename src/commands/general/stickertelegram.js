...
async function stickerTelegramCommand(sock, chatId, msg) {
    try {
        ...
        try {
            ...
            await sock.sendMessage(chatId, { 
                text: `✅🌿❄𝐄𝐌𝐌𝐘𝐇𝐄𝐍𝐙-𝐕2❄🌿 Successfully downloaded ${successCount}/${stickerSet.result.stickers.length} stickers!` 
            });

        } catch (error) {
            throw new Error(`Failed to process sticker pack: ${error.message}`, { cause: error });
        }
